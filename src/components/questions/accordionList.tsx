...
export default function AccordionList({ accordionList }: AccordionListProps) {
  ...
  return (
    <Accordion data-aos="fade-up" className="mb-[20px] sm:mb-[50px]">
      {accordionList.map((data: QAInterface) => (
        <AccordionItem
          key={data.id}
          aria-label={""}
          title={
            <p className="text-right px-[5px] py-2 sm:py-3">{data.question}</p>
          }
          className=" text-sm font-extrabold text-gray-300  hover:bg-white hover:bg-opacity-10 sm:text-xl"
        >
